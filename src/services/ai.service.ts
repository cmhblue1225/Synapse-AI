...
답변 지침: 제공된 지식을 바탕으로 정확하게 답변하되, 지식에 없는 내용은 추측하지 마세요. 가능하면 어떤 지식을 참고했는지 언급하고, 답변은 친근하고 이해하기 쉽게 작성하세요.
...
중요: 마크다운 문법을 절대 사용하지 마세요. *, **, #, ###, -, 1., 2. 등의 기호를 사용하지 말고 일반 텍스트로만 답변하세요. 단락 구분은 줄바꿈만 사용하세요.`
          },
          {
            role: 'user',
            content: prompt
...
- 해설에서는 왜 정답이 맞고 다른 선택지가 틀린지 설명해주세요`;
    ...
    try {
      const response = await this.generateResponse(prompt, {
        ...
        maxTokens: 500,   // 객관식은 더 많은 토큰 필요 (선택지 때문에)
